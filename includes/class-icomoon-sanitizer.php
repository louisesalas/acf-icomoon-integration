...
class ACF_IcoMoon_Sanitizer {
    ...
    public function sanitize_svg( string $svg_content ) {
        if ( empty( $svg_content ) ) {
            return new WP_Error( 'empty_svg', __( 'SVG content is empty.', 'acf-icomoon' ) );
        }
        ...
        if ( preg_match( '/<!DOCTYPE/i', $svg_content ) ) {
            return new WP_Error( 
                'doctype_not_allowed', 
                __( 'SVG files with DOCTYPE declarations are not allowed for security reasons.', 'acf-icomoon' ) 
            );
        }
        ...
        if ( preg_match( '/<!ENTITY/i', $svg_content ) ) {
            return new WP_Error( 
                'entity_not_allowed', 
                __( 'SVG files with ENTITY declarations are not allowed for security reasons.', 'acf-icomoon' ) 
            );
        }
        ...
        if ( ! $loaded ) {
            ...
            return new WP_Error( 
                'invalid_svg', 
                __( 'Invalid SVG file. Could not parse XML content.', 'acf-icomoon' ) 
            );
        }
        ...
        if ( false === $sanitized ) {
            return new WP_Error( 
                'sanitization_failed', 
                __( 'Failed to sanitize SVG content.', 'acf-icomoon' ) 
            );
        }
    ...
    public function validate_svg_file( string $file_path ) {
        if ( ! file_exists( $file_path ) ) {
            return new WP_Error( 'file_not_found', __( 'File not found.', 'acf-icomoon' ) );
        }

        $content = file_get_contents( $file_path );
        
        if ( false === $content ) {
            return new WP_Error( 'file_read_error', __( 'Could not read file.', 'acf-icomoon' ) );
        }
        ...
        if ( $size > $max_size ) {
            return new WP_Error( 
                'file_too_large', 
                __( 'SVG file is too large. Maximum size is 5MB.', 'acf-icomoon' ) 
            );
        }
        ...
        if ( ! preg_match( '/<svg[^>]*>/i', $content ) ) {
            return new WP_Error( 
                'not_svg', 
                __( 'File does not appear to be a valid SVG.', 'acf-icomoon' ) 
            );
        }
        ...
        if ( preg_match( '/<script/i', $content ) ) {
            return new WP_Error( 
                'script_not_allowed', 
                __( 'SVG files containing script tags are not allowed.', 'acf-icomoon' ) 
            );
        }
